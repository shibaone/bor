...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := ethapi.DoCall(ctx, b.r.backend, args.Data, *b.numberOrHash, nil, nil, nil, b.r.backend.RPCEVMTimeout(), b.r.backend.RPCGasCap(), true, true)
	if err != nil {
		return nil, err
	}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := ethapi.DoCall(ctx, p.r.backend, args.Data, pendingBlockNr, nil, nil, nil, p.r.backend.RPCEVMTimeout(), p.r.backend.RPCGasCap(), true, true)
	if err != nil {
		return nil, err
	}
	status := hexutil.Uint64(1)
